...
router.post('/getcompanycode', function(req, res, next) {
    ...
    let mailOptions = {
        ...
        to: req.body.email,
        subject: 'Registratiecode voor uw bedrijf',
        text: 'Hierbij uw code om eenmalig een account aan te kunnen maken: ' + code
      };
    ...
    if (!req.body.password) {
        ...
        bcrypt.hash(req.body.password, 10, function (err, hash) {
            database.query("UPDATE gebruiker SET wachtwoord = ? WHERE email = ?", [hash, req.body.email]).then(rows => {
                return database.query("UPDATjE wachtwoordcode SET geldig = 0 WHERE gebruikeremail = ?", [req.body.email]);
            }).then(rows => {
